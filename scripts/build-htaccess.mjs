...
/**
 * public/_redirects is an input, so Astro copies it to dist and it was being
 * served at https://sgualda.com/_redirects — a plain-text list of every rule,
 * published because the file happened to live in the folder that gets copied.
 * Nothing in it is secret, but shipping a build artefact is how a directory
 * stops being a description of the site. Its content is already compiled into
 * the .htaccess two lines above.
 */
const stray = join(dist, '_redirects');
if (existsSync(stray)) {
  unlinkSync(stray);
  console.log('  · dropped dist/_redirects — build input, not a public file');
}
